...
def _make_http_result(status_code: int = 200, elapsed_ms: int = 100) -> HttpResult:
    return HttpResult(status_code=status_code, elapsed_ms=elapsed_ms)
...
def test_execute_happy_path_with_ssl_check():
    # Arrange: probe that requests cert info
    probe = Probe(name="p1", url="https://example.com", schedule="0 0 * * *", checkCert=True)
    ...
    assert cast(AsyncMock, publisher.publish).await_count == 1

    first_call = cast(AsyncMock, publisher.publish).await_args_list[0]
    assert first_call.args[0] == probe.name
    assert first_call.args[1].status_code == 200
    assert first_call.args[2].subject_cn == "example.com"


def test_execute_no_ssl_check():
    # Arrange: probe without cert check
    probe = Probe(name="p2", url="https://no-ssl.example", schedule="0 * * * *", checkCert=False)
...
def test_execute_response_err_stops_processing():
    # Arrange
    probe = Probe(name="p3", url="https://down.example", schedule="0 0 * * *", checkCert=True)
...
def test_execute_cert_err_stops_processing():
    # Arrange: probe with cert check where cert retrieval fails
    probe = Probe(name="p4", url="https://badcert.example", schedule="0 0 * * *", checkCert=True)
